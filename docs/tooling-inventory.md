...
- Einsicht in Projektdateien wie [`build.ps1`](D:\Codex\TrafficView_Moi\TrafficView%201.4.12\build.ps1)
...
| ComfyUI | Zusatzsoftware | Nachinstalliert / eingerichtet | `0.18.2` | `D:\Codex\!Grafikhilfen\ComfyUI\ComfyUI` | `D:\Codex\!Grafikhilfen\ComfyUI\ComfyUI\main.py` | lokale Installation unter `D:\Codex\!Grafikhilfen\ComfyUI`; Git-Remote zeigt `https://github.com/Comfy-Org/ComfyUI` | Zentrale Bildgenerierung und `img2img` für TrafficView-Skins | Ja, bevorzugt |
| ComfyUI Embedded Python | Zusatzsoftware | Mit ComfyUI installiert / verwendet | `3.13.11` | `D:\Codex\!Grafikhilfen\ComfyUI\python_embeded` | `D:\Codex\!Grafikhilfen\ComfyUI\python_embeded\python.exe` | Bestandteil der ComfyUI-Installation, lokal per `--version` geprüft | Bevorzugte Laufzeit zum Starten von ComfyUI | Ja |
...
| SDXL Base 1.0 Checkpoint | Modellpaket | Verwendet | unbekannt | `D:\Codex\!Grafikhilfen\Models\checkpoints` | `D:\Codex\!Grafikhilfen\Models\checkpoints\sd_xl_base_1.0.safetensors` | lokal vorhanden; Download-Herkunft in dieser Datei nicht sicher feststellbar | Hauptmodell für Skin-Rendering in ComfyUI | Ja |
| SDXL VAE | Modellpaket | Verwendet / vorhanden | unbekannt | `D:\Codex\!Grafikhilfen\Models\vae` | `D:\Codex\!Grafikhilfen\Models\vae\sdxl_vae.safetensors` | lokal vorhanden; Download-Herkunft nicht sicher feststellbar | VAE für SDXL-Decoding in ComfyUI | Ja |
| ControlNet Canny | Modellpaket | Nachinstalliert / vorhanden | unbekannt | `D:\Codex\!Grafikhilfen\Models\controlnet` | `D:\Codex\!Grafikhilfen\Models\controlnet\control_v11p_sd15_canny_fp16.safetensors` | im Projektverlauf nachinstalliert; genaue Downloadquelle hier nicht verifiziert | Für künftige Kanten-/Konturstabilisierung bei Skin-Arbeit vorgesehen | Ja |
| TrafficView Model Path Mapping | Hilfskonfiguration | Nachinstalliert / eingerichtet | unbekannt | `D:\Codex\!Grafikhilfen\ComfyUI\ComfyUI` | `D:\Codex\!Grafikhilfen\ComfyUI\ComfyUI\extra_model_paths.yaml` | lokal angelegt, Inhalt geprüft | Bindet zentrale Modellordner (`checkpoints`, `vae`, `loras`, `controlnet`) in ComfyUI ein | Ja |
...
- `IPAdapter`-Modelle:
  - geprüft über `D:\Codex\!Grafikhilfen\Models\ipadapter`
...
1. Für Skin-Erzeugung zuerst ComfyUI unter `D:\Codex\!Grafikhilfen\ComfyUI` prüfen und wiederverwenden.
2. Modelle zuerst unter `D:\Codex\!Grafikhilfen\Models` suchen, bevor neue Downloads begonnen werden.
3. Bereits vorhandene Workflow-Helfer unter `D:\Codex\!Grafikhilfen\Workflows` bevorzugt wiederverwenden.
4. TrafficView weiterhin über [`build.ps1`](D:\Codex\TrafficView_Moi\TrafficView%201.4.12\build.ps1) und `csc.exe` bauen, nicht primär über `dotnet`.
5. Vor neuer Zusatzsoftware immer diese Datei prüfen, um Redundanzen zu vermeiden.
