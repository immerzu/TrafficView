...
def clear_right_circle_zone(image: Image.Image) -> Image.Image:
    ...
    outer_r = live_radius + (height * 0.102)
    mid_r = live_radius + (height * 0.028)
    inner_r = max(1.0, live_radius - (height * 0.034))

    clear_layer = new_layer(image.size)
    cdraw = ImageDraw.Draw(clear_layer)
    cdraw.ellipse(
        (
            cx - outer_r - (height * 0.18),
            cy - outer_r - (height * 0.18),
            cx + outer_r + (height * 0.18),
            cy + outer_r + (height * 0.18),
        ),
        fill=(6, 16, 30, 232),
    ...
    bdraw.rounded_rectangle(
        (
            cx - outer_r - (width * 0.17),
            cy - beam_h / 2.0,
            min(width - 3.0, cx + outer_r + (width * 0.12)),
            cy + beam_h / 2.0,
        ),
