...
def clear_right_circle_zone(image: Image.Image) -> Image.Image:
    ...
    outer_r = int(round(height * 0.315))
    mid_r = int(round(height * 0.245))
    inner_r = int(round(height * 0.165))

    clear_layer = new_layer(image.size)
    cdraw = ImageDraw.Draw(clear_layer)
    cdraw.ellipse(
        (cx - outer_r - 9, cy - outer_r - 9, cx + outer_r + 10, cy + outer_r + 10),
        fill=(6, 16, 30, 232),
    )
    cdraw.rounded_rectangle(
        (
            cx + int(round(width * 0.09)),
            cy + int(round(height * 0.07)),
            width - 1,
            height - 2,
        ),
    ...
    bdraw.rounded_rectangle(
        (
            cx - outer_r - int(round(width * 0.18)),
            cy - beam_h // 2,
            min(width - 4, cx + outer_r + int(round(width * 0.13))),
            cy + beam_h // 2,
        ),
